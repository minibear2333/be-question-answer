...
class DWQA_Ajax {
	...
	public function delete_comment() {
		if ( ! isset( $_GET['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( $_GET['_wpnonce'] ), '_dwqa_delete_comment' ) ) {
			wp_die( __( 'Are you cheating huh?', 'be-question-answer' ) );
		}

		if ( !dwqa_current_user_can( 'delete_comment' ) ) {
			wp_die( __( 'You do not have permission to edit comment.', 'be-question-answer' ) );
		}

		if ( ! isset( $_GET['comment_id'] ) ) {
			wp_die( __( 'Comment ID must be showed.', 'be-question-answer' ) );
		}
	...
	public function flag_answer() {
		...
		if ( ! isset( $_POST['answer_id'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Missing id of answer', 'be-question-answer' ) ) );
		}
		global $current_user;
	...
	public function delete_question() {
		...
		if ( ! isset( $_GET['question_id'] ) ) {
			wp_die( __( 'Question is missing.', 'be-question-answer' ), 'error' );
		}

		if ( 'dwqa-question' !== get_post_type( intval( $_GET['question_id'] ) ) ) {
			wp_die( __( 'This post is not question.', 'be-question-answer' ) );
		}
	...
	public function update_status() {
		...
		if ( dwqa_current_user_can( 'edit_question' ) || $current_user->ID == $question->post_author ) {
			...
			wp_send_json_error( array(
				'message'   => __( 'You do not have permission to edit question status', 'be-question-answer' )
			) );
		}
	}
	...
	public function auto_suggest_for_seach(){
		if ( ! isset( $_POST['nonce'])  ) {
			wp_send_json_error( array( array( 
				'error' => 'sercurity',
				'message' => __( 'Are you cheating huh?', 'be-question-answer' ) 
			) ) );
		}
		...
		if ( ! isset( $_POST['title'] ) ) {
			wp_send_json_error( array( array( 
				'error' => 'empty title',
				'message' => __( 'Not Found!!!', 'be-question-answer' ), 
			) ) );
		}
		...
		if ( $query->have_posts() ) {
			...
			wp_reset_query();
			wp_send_json_error( array( array( 'error' => 'not found', 'message' => __( 'Not Found!!!', 'be-question-answer' ) ) ) );
		}
	}
