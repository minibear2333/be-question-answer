...
class DWQA_Answer_List_Table extends WP_List_Table {
	...
	function get_columns() {
		$columns = array( 
			...
			'detail'    => __( 'Detail', 'be-question-answer' ),
		);
		return $columns;
	}
	...
	function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			case 'author':
				...
				if ( ! $user_info ) {
					echo '<strong>'.__( 'Anonymous','be-question-answer' ).'</strong>';
				} else {
			...
			case 'detail':
				...
				setup_postdata( $item );
				?>
				<div class="submitted-on"><?php _e( 'Answered on ', 'be-question-answer' ) ?><a href="<?php echo get_permalink( $item->ID ) ?>"><?php echo $item->post_date ?></a></div>
				<?php the_excerpt(); ?>
				<?php
				break;
			default:
	...
	function trim_exceprt_more( $excerpt ) {
		if ( $excerpt ) {
			return str_replace( '[...]', '<a href="'.get_permalink().'" title="'.__( 'Read more', 'be-question-answer' ).'" >...</a>', $excerpt ); 
		}
	}
